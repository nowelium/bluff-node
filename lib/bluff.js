...
Bluff.merge(Bluff.Renderer.prototype, {
  ...
  annotate_scaled: function(width, height, x, y, text, scale) {
    ...
    var posY = (this._sx * x + this._left_adjustment(text, scaled_width));
  ...
  _left_adjustment: function(node, width) {
    var w = this._sized_text(node).width;
    switch (this.gravity) {
    case 'west':
      return 0;
    case 'east':
  ...
  _top_adjustment: function(node, height) {
    var h = this._sized_text(node).height || 0;
  ...
  _sized_text: function(size, content) {
    this._ctx.save();
    try {
      var fontWeight = this.font_weight;
