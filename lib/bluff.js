...
Bluff.merge(Bluff.Base.prototype, {
  ...
  initialize: function(renderer, target_width) {
    target_width = target_width || this.DEFAULT_TARGET_WIDTH;
    ...
    this._d = new Bluff.Renderer(renderer, this._columns, this._rows);
  ...
  initialize_ivars: function() {
    ...
    this.left_margin = this.right_margin = this.DEFAULT_MARGIN;
    
    this.legend_margin = this.LEGEND_MARGIN;
    this.title_margin = this.TITLE_MARGIN;
    ...
    this._d.default_font_size = this.default_font_size;
  },
  ...
  //*
  set_default_font_size: function(font_size){
    this.default_font_size = font_size;
    this._d.default_font_size = font_size;
  },
  ...
  set_theme: function(options) {
    ...
    for (var key in options) {
      this._theme_options[key] = options[key];
    }
  ...
  _setup_drawing: function() {
    ...
    if (!this._has_data) {
      return this._draw_no_data();
    }
    
    this._normalize();
    this._setup_graph_measurements();
    if (this.sort) {
      this._sort_norm_data();
    }
  ...
  _normalize: function(force) {
    if (this._norm_data === null || force === true) {
      ...
      Bluff.each(this._data, function(data_row) {
        ...
        Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point) {
        ...
        this._norm_data.push([data_row[this.DATA_LABEL_INDEX], norm_data_points, data_row[this.DATA_COLOR_INDEX]]);
      }, this);
    }
  },
  ...
  _setup_graph_measurements: function() {
    ...
    if (this.hide_line_markers) {
      ...
      if (this.has_left_labels) {
        ...
        //* TODO:
        // osx Calculates
        //  longest_left_label_width to 0
        // linux calculates
        //  longest_left_label_width to 1.94875647e-316
        if('' != longest_label){
      ...
      0.0 :
        longest_left_label_width + this.LABEL_MARGIN * 2;
      
      this._graph_left = this.left_margin +
        line_number_width +
        (this.y_axis_label === null ? 0.0 : this._marker_caps_height + this.LABEL_MARGIN * 2);
      ...
      this._graph_bottom_margin = this.bottom_margin +
        this._marker_caps_height + this.LABEL_MARGIN;
    }
  ...
  _draw_axis_labels: function() {
    if (this.x_axis_label) {
      ...
      var x_axis_label_y_coordinate = this._graph_bottom + this.LABEL_MARGIN * 2 + this._marker_caps_height;
  ...
  _draw_line_markers: function() {
    ...
    for (index = 0, n = this.marker_count; index <= n; index++) {
      ...
      if (!this.hide_line_numbers) {
        ...
        this._d.annotate_scaled(this._graph_left - this.LABEL_MARGIN,
                                1.0, 0.0, y,
  ...
  _draw_legend: function() {
    ...
    this._legend_labels = Bluff.map(this._data, function(item) {
      return item[this.DATA_LABEL_INDEX];
    }, this);
    ...
    Bluff.each(this._legend_labels, function(legend_label, index) {
      ...
      this._d.stroke = 'transparent';
      this._d.fill = this._data[index][this.DATA_COLOR_INDEX];
      ...
      if (label_widths[0].length == 0) {
        ...
        if (label_widths.length > 0) {
          current_x_offset = this._center(Bluff.sum(label_widths[0]));
        }
  ...
  _draw_label: function(x_offset, index) {
    ...
    if (this.labels[index] && !this._labels_seen[index]) {
      y_offset = this._graph_bottom + this.LABEL_MARGIN;
  ...
  _sort_norm_data: function() {
    var sums = this._sums, index = this.DATA_VALUES_INDEX;
  ...
  _make_stacked: function() {
    ...
    Bluff.each(this._data, function(value_set) {
      Bluff.each(value_set[this.DATA_VALUES_INDEX], function(value, index) {
        stacked_values[index] += value;
      }, this);
      value_set[this.DATA_VALUES_INDEX] = Bluff.array(stacked_values);
    }, this);
  },
  ...
  _label: function(value) {
    var sep   = this.THOUSAND_SEPARATOR,
...
Bluff.Area = function (){
  Bluff.Base.apply(this, arguments);
};
Bluff.merge(Bluff.Area.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Area.prototype, {
  draw: function() {
    Bluff.Base.prototype.draw.call(this);
    ...
    Bluff.each(this._norm_data, function(data_row) {
      ...
      Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point, index) {
      ...
      this._d.fill = data_row[this.DATA_COLOR_INDEX];
...
Bluff.merge(Bluff.BarConversion.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.BarConversion.prototype, {
...
Bluff.merge(Bluff.Bar.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Bar.prototype, {
  draw: function() {
  ...
  _draw_bars: function() {
    ...
    Bluff.each(this._norm_data, function(data_row, row_index) {
      var raw_data = this._data[row_index][this.DATA_VALUES_INDEX];
      
      Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point, point_index) {
        // Use incremented x and scaled y
        // x
...
Bluff.merge(Bluff.Line.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Line.prototype, {
  ...
  initialize: function(renderer) {
    ...
    if (arguments.length === 1 || (typeof arguments[1] !== 'number' && typeof arguments[1] !== 'string')){
      Bluff.Base.prototype.initialize.call(this, renderer);
    } else {
      Bluff.Base.prototype.initialize.apply(this, arguments);
    }
  ...
  draw: function() {
    ...
    Bluff.each(this._norm_data, function(data_row, row_index) {
      ...
      Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point, index) {
        ...
        this._d.stroke = data_row[this.DATA_COLOR_INDEX];
        this._d.fill = data_row[this.DATA_COLOR_INDEX];
        this._d.stroke_opacity = 1.0;
        this._d.stroke_width = this.line_width ||
          this._clip_value_if_greater_than(this._columns / (this._norm_data[0][this.DATA_VALUES_INDEX].length * 6), 3.0);
        
        var circle_radius = this.dot_radius ||
          this._clip_value_if_greater_than(this._columns / (this._norm_data[0][this.DATA_VALUES_INDEX].length * 2), 7.0);
  ...
  _normalize: function() {
    ...
    Bluff.Base.prototype._normalize.call(this);
...
Bluff.merge(Bluff.Dot.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Dot.prototype, {
  draw: function() {
  ...
  _draw_line_markers: function() {
    ...
    for (var index = 0; index <= number_of_lines; index++) {
      ...
      this._d.stroke = this.marker_color;
      this._d.line(x, this._graph_bottom, x, this._graph_bottom + 0.5 * this.LABEL_MARGIN);
      ...
      if (!this.hide_line_numbers) {
        ...
        // Width of box to draw text in
        this._d.annotate_scaled(0, 0,
                                x, this._graph_bottom + (this.LABEL_MARGIN * 2.0), // Coordinates of text
  ...
  _draw_label: function(y_offset, index) {
    if (this.labels[index] && !this._labels_seen[index]) {
      ...
      this._d.annotate_scaled(1, 1,
                              this._graph_left - this.LABEL_MARGIN * 2.0, y_offset,
                              this.labels[index], this._scale);
      this._labels_seen[index] = true;
    }
  }
...
Bluff.merge(Bluff.Net.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Net.prototype, {
  initialize: function() {
    Bluff.Base.prototype.initialize.apply(this, arguments);
  ...
  draw: function() {
    ...
    this._x_increment = this._graph_width / (this._column_count - 1);
    var circle_radius = this.dot_radius ||
      this._clip_value_if_greater_than(this._columns / (this._norm_data[0][this.DATA_VALUES_INDEX].length * 2.5), 7.0);
    
    this._d.stroke_opacity = 1.0;
    this._d.stroke_width = this.line_width ||
      this._clip_value_if_greater_than(this._columns / (this._norm_data[0][this.DATA_VALUES_INDEX].length * 4), 3.0);
    ...
    Bluff.each(this._norm_data, function(data_row) {
      ...
      Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point, index) {
        ...
        var rad_pos = index * Math.PI * 2 / this._column_count,
            ...
            next_index = (index + 1 < data_row[this.DATA_VALUES_INDEX].length) ? index + 1 : 0,
            
            next_rad_pos = next_index * Math.PI * 2 / this._column_count,
            next_point_distance = data_row[this.DATA_VALUES_INDEX][next_index] * this._radius,
        ...
        if (!this.hide_dots) {
          this._d.circle(start_x, start_y, start_x - circle_radius, start_y);
        }
      }, this);
...
Bluff.merge(Bluff.Pie.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Pie.prototype, {

  TEXT_OFFSET_PERCENTAGE: 0.08,
 
  initialize_ivars: function() {
    Bluff.Base.prototype.initialize_ivars.call(this);
  ...
  draw: function() {
    ...
    var diameter = this._graph_height,
        ...
        index = this.DATA_VALUES_INDEX;
    
    // Use full data since we can easily calculate percentages
    if (this.sort) this._data.sort(function(a,b) { return a[index][0] - b[index][0]; });
    Bluff.each(this._data, function(data_row, i) {
      if (data_row[this.DATA_VALUES_INDEX][0] > 0) {
        this._d.fill = data_row[this.DATA_COLOR_INDEX];
        
        var current_degrees = (data_row[this.DATA_VALUES_INDEX][0] / total_sum) * 360;
        ...
        var half_angle = prev_degrees + ((prev_degrees + current_degrees) - prev_degrees) / 2,
            label_val = Math.round((data_row[this.DATA_VALUES_INDEX][0] / total_sum) * 100.0),
            label_string;
        
        if (label_val >= this.hide_labels_less_than) {
          label_string = this._label(data_row[this.DATA_VALUES_INDEX][0]);
          this._draw_label(center_x, center_y, half_angle,
                            radius + (radius * this.TEXT_OFFSET_PERCENTAGE),
                            label_string,
                            data_row, i);
        }
  ...
  _sums_for_pie: function() {
    var total_sum = 0;
    Bluff.each(this._data, function(data_row) {
      total_sum += data_row[this.DATA_VALUES_INDEX][0];
    }, this);
    return total_sum;
  }
});
...
Bluff.merge(Bluff.SideBar.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.SideBar.prototype, {
  draw: function() {
    this.has_left_labels = true;
    Bluff.Base.prototype.draw.call(this);
  ...
  _draw_line_markers: function() {
    ...
    for (var index = 0; index <= number_of_lines; index++) {
      ...
      x            = this._graph_right - (line_diff * index) - 1;
      diff         = index - number_of_lines;
      marker_label = Math.floor(Math.abs(diff) * increment + this.minimum_value);
      ...
      if (!this.hide_line_numbers) {
        ...
        this._d.annotate_scaled(
          0, 0,
          // Coordinates of text
          x, this._graph_bottom + (this.LABEL_MARGIN * 2.0),
          this._label(marker_label), this._scale
        );
      }
    }
...
Bluff.merge(Bluff.Spider.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.Spider.prototype, {
  initialize: function(renderer, max_value, target_width) {
    Bluff.Base.prototype.call(this, renderer, target_width);
  ...
  draw: function() {
    ...
    var diameter = this._graph_height,
        ...
        center_x = this._graph_left + (this._graph_width / 2.0),
        // Move graph up a bit
        center_y = this._graph_top + (this._graph_height / 2.0) - 25;
  ...
  _draw_axes: function(center_x, center_y, radius, additive_angle, line_color) {
    ...
    Bluff.each(this._data, function(data_row) {
      this._d.stroke = line_color || data_row[this.DATA_COLOR_INDEX];
      ...
      if (!this.hide_text) this._draw_label(center_x, center_y, current_angle, radius, data_row[this.DATA_LABEL_INDEX]);
  ...
  _draw_polygon: function(center_x, center_y, additive_angle, color) {
    var points = [],
        current_angle = 0.0;
    Bluff.each(this._data, function(data_row) {
      points.push(center_x + this._normalize_points(data_row[this.DATA_VALUES_INDEX][0]) * Math.cos(current_angle));
      points.push(center_y + this._normalize_points(data_row[this.DATA_VALUES_INDEX][0]) * Math.sin(current_angle));
      current_angle += additive_angle;
    }, this);
  ...
  _sums_for_spider: function() {
    var sum = 0.0;
    Bluff.each(this._data, function(data_row) {
      sum += data_row[this.DATA_VALUES_INDEX][0];
    }, this);
    return sum;
  }
});
...
Bluff.Base.StackedMixin = function (){};
Bluff.merge(Bluff.Base.StackedMixin.prototype, {
  // Get sum of each stack
  _get_maximum_by_stack: function() {
    var max_hash = {};
    Bluff.each(this._data, function(data_set) {
      Bluff.each(data_set[this.DATA_VALUES_INDEX], function(data_point, i) {
...
Bluff.StackedArea = function(){
  Bluff.merge(this, {
    last_series_goes_on_bottom: null
  });
  Bluff.Base.apply(this, arguments);
};
Bluff.merge(Bluff.StackedArea.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.StackedArea.prototype, Bluff.merge(new Bluff.Base.StackedMixin, {
  draw: function() {
...
Bluff.merge(Bluff.StackedBar.prototype, Bluff.Base.prototype);
Bluff.merge(Bluff.StackedBar.prototype, Bluff.merge(new Bluff.Base.StackedMixin, {
...
Bluff.AccumulatorBar = function (){
  Bluff.StackedBar.apply(this, arguments);
};
Bluff.merge(Bluff.AccumulatorBar.prototype, Bluff.StackedBar.prototype);
Bluff.merge(Bluff.AccumulatorBar.prototype, {
  draw: function() {
    ...
    Bluff.each(this._data[0][this.DATA_VALUES_INDEX], function(value) {
      var max = -Infinity;
    ...
    Bluff.StackedBar.prototype.draw.call(this);
  }
});
...
Bluff.SideStackedBar = function (){
  Bluff.merge(this, {
    // Spacing factor applied between bars
    bar_spacing: 0.9
  });
  Bluff.SideBar.apply(this, arguments);
};
Bluff.merge(Bluff.SideStackedBar.prototype, Bluff.SideBar.prototype);
Bluff.merge(Bluff.SideStackedBar.prototype, Bluff.merge(new Bluff.Base.StackedMixin, {
  draw: function() {
    this.has_left_labels = true;
    this._get_maximum_by_stack();
    Bluff.SideBar.prototype.draw.call(this);
  },
  ...
  _draw_bars: function() {
    ...
    Bluff.each(this._norm_data, function(data_row, row_index) {
      ...
      Bluff.each(data_row[this.DATA_VALUES_INDEX], function(data_point, point_index) {
        ...
        //* no draw stackedbar then diff < 0
        if(difference < 0){
          this._d.fill = data_row[this.DATA_COLOR_INDEX];
  ...
  _max: function(data_point, index) {
    ...
    Bluff.each(this._data, function(item) {
      sum += item[this.DATA_VALUES_INDEX][index];
    }, this);
    return sum;
  }
}));

Bluff.Mini.Legend = function (){
  Bluff.merge(this, {
    hide_mini_legend: false
  });
};
Bluff.merge(Bluff.Mini.Legend.prototype, {
...
//
Bluff.Mini.Bar = function (){
  Bluff.Bar.apply(this, arguments);
};
Bluff.merge(Bluff.Mini.Bar.prototype, Bluff.Bar.prototype);
Bluff.merge(Bluff.Mini.Bar.prototype, Bluff.merge(new Bluff.Mini.Legend, {
  initialize_ivars: function() {
    Bluff.Bar.prototype.initialize_ivars.call(this);
  ...
  draw: function() {
    ...
    Bluff.Bar.prototype.draw.call(this);
...
//
Bluff.Mini.Pie = function (){
  Bluff.Pie.apply(this, arguments);
};
Bluff.merge(Bluff.Mini.Pie.prototype, Bluff.Pie.prototype);
Bluff.merge(Bluff.Mini.Pie.prototype, Bluff.merge(new Bluff.Mini.Legend, {
  initialize_ivars: function() {
    Bluff.Pie.prototype.initialize_ivars.call(this);
  ...
  draw: function() {
    ...
    Bluff.Pie.prototype.draw.call(this);
...
// Makes a small pie graph suitable for display at 200px or even smaller.
//
Bluff.Mini.SideBar = function (){
  Bluff.SideBar.apply(this, arguments);
};
Bluff.merge(Bluff.Mini.SideBar.prototype, Bluff.SideBar.prototype);
Bluff.merge(Bluff.Mini.SideBar.prototype, Bluff.merge(new Bluff.Mini.Legend, {
  initialize_ivars: function() {
    Bluff.SideBar.prototype.initialize_ivars.call(this);
    this.hide_legend = true;
    this.hide_title = true;
    this.hide_line_numbers = true;
...
Bluff.merge(Bluff.Renderer.prototype, {
  ...
  caps_height: function(font_size) {
    var text = this._sized_text(font_size, 'X');
    return text.height || 24;
  },
  
  text_width: function(font_size, textContent) {
    var text = this._sized_text(font_size, textContent);
    return text.width;
  },
  
  get_type_metrics: function(textContent) {
    var text = this._sized_text(this.pointsize, textContent);
    return {
      width: text.width,
  ...
  annotate_scaled: function(width, height, x, y, text, scale) {
    ...
    //* TODO: hard coded number:: top margin
    var topMargin = 3;

    var posX = (this._sy * y + this._top_adjustment(text, scaled_height)) + topMargin;
    var posY = (this._sx * x + this._left_adjustment(text, scaled_width, this.pointsize));

    if(posY < 1){
      posY = Math.abs(posY);
    }
    //var sizedText = this._sized_text(this.pointsize, text);

    this._ctx.save();
    try {
      var fontWeight = this.font_weight || 'normal';
      var fontFamily = this.font;
      var fontSize = (typeof this.pointsize === 'number') ? this.pointsize + 'px' : pointsize;

      this._ctx.font = fontWeight + ' ' + fontSize + ' ' + fontFamily;
      this._ctx.fillStyle = this.fill;
      this._ctx.fillText(text, posY, posX);
    } finally {
      this._ctx.restore();
    }
  },
  ...
  _left_adjustment: function(node, width, fontsize) {
    fontsize = fontsize || 24;
    var w = this._sized_text(fontsize, node).width;
    switch (this.gravity) {
    case 'west':
      return 0;
    case 'east':
  ...
  _top_adjustment: function(node, height) {
    var h = this._sized_text(24, node).height || 0;
  ...
  _sized_text: function(size, content) {
    ...
    try {
      var fontWeight = this.font_weight || 'normal';
      var fontFamily = this.font;
      var fontSize = (typeof size === 'number') ? size + 'px' : size;

      this._ctx.font = fontWeight + ' ' + fontSize + ' ' + fontFamily;
      return this._ctx.measureText(content);
    } finally {
      this._ctx.restore();
    }
  }
